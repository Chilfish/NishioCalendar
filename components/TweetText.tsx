...
export const TweetText: React.FC<TweetTextProps> = ({ text }) => {
  ...
  if (replyToName) {
    return (
      <>
        <p className="py-2 wrap-anywhere text-sm">
          回复
          {' '}
          <PeopleLink name={replyToName} />
          :
        </p>
        <p className="wrap-anywhere text-sm">{parseTweetText(content || '')}</p>
      </>
    )
  }

  return <p className="pt-2 wrap-anywhere text-sm">{parseTweetText(text)}</p>
}
